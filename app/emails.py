...
def ask_notification(friend, user, ask):
    send_email(
        "[HelpJM] %s %s needs good %s services ?" %
...
def thankyou_notification(friend, user, recommendation, message):
    send_email(
        "[HelpJM] %s %s is thanking you for the Recommendation that you sent." %
        (user.firstname,
         user.lastname),
        ADMINS[0],
        [
...
def recommendation_notification(friend, user, ask, recommendationid):
    send_email(
        "[HelpJM] %s %s has a recommendation for you" %
        (friend.firstname,
         friend.lastname),
        ADMINS[0],
        [
...
def recommendation_notification2(friend, user, ask, recList):
    send_email(
        "[HelpJM] %s %s has recommendations for you" %
        (friend.firstname,
         friend.lastname),
        ADMINS[0],
        [
...
def user_confirmation_notification(user):
    send_email(
        "[HelpJM] Registration Confirmation for %s %s " %
